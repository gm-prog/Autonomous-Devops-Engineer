...
class GitHubPRClient:
    ...
    def create_pull_request(self, repo_slug: str, branch: str, title: str, body: str, draft: bool = True) -> str:
        ...
        if not self.oauth_token:
            # The previous implementation silently returned a fabricated PR
            # URL here, letting operations "succeed" in the logs with no real
            # PR. Fail loudly instead: a missing credential is an error.
            raise InvalidGitHubTokenException(
                "GITHUB_OAUTH_TOKEN is not configured; refusing to report a "
                "fabricated Pull Request URL as success."
            )
